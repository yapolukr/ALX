        ...
        for i in self.listaFirm:
            if i.nazwa == nazwa:
                if (len(i.listaPracovnik) == 0):
                    ...
                    print("Nie można usunąć firmy, ponieważ zawiera zatrudnionych pracowników")
    ...
    def usunPrac(self, nazwisko):
        for i in self.listaPracownik:
             if i.nazwisko == nazwisko:
                 if (len(i.listaFirm) == 0):
                     self.listaPracownik.remove(i)
                     break
                 else:
                     print("Nie można usunąć pracownika, ponieważ pracuje w firmie")

    def wyswietl(self):
        for i in self.listaPracownik:
            print(f"Imie: {i.imie}, Nazwisko: {i.nazwisko}")
            for j in i.listaFirm:
                print(f"Firma: {j.nazwa}")
        ...
        for i in self.listaPracownik:
            if i.nazwisko == nazwisko:
                ...
                for j in self.listaFirm:
                    if j.nazwa == nazwa:
                        i.listaFirm.append(j)
                        j.listaPracovnik.append(i)

                        self.zapisDopliku()
                        break

    def zatrudnenie(self, nazwisko):
        for i in self.listaPracownik:
            if i.nazwisko == nazwisko:
                for j in i.listaFirm:
                    print(j.nazwa)
